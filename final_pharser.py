...
class PhrankPipeline:
    def __init__(self, dag_file, disease_data):
        ...
        # Compute information content (IC) and marginal IC.
        # Here, each disease is treated as if it were a gene.
        self._IC, self._marginal_IC = Phrank.compute_information_content(self._disease_pheno_map, self._child_to_parent)
    ...
    def rank_diseases(self, patient_phenotypes, threshold=0.2):
        ...
        for disease, disease_phenos in self._disease_pheno_map.items():
            ...
            # Intersection and union of the closures
            intersection = patient_closure & disease_closure
            union = patient_closure | disease_closure
            
            # Sum the marginal IC values for the intersection and union
            inter_score = sum(self._marginal_IC.get(pheno, 0) for pheno in intersection)
            union_score = sum(self._marginal_IC.get(pheno, 0) for pheno in union)
            
            # Compute weighted Jaccard similarity
            score = inter_score / union_score if union_score > 0 else 0
            results.append((disease, score))
...
def test_phrank_pipeline(user_symptom):
    """
    Test the Phrank pipeline:
      1) Load disease data from 'data/converted_orphanet.json'.
    ...
    # patient_symptoms = input("Please mention your symptoms: \n")
    print("LULE")
    patient_phenotypes = load_hpo_terms(user_symptom)   
